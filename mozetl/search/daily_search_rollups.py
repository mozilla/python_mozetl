...
def with_shares(dataframe):
    ...
    Given a single profile_day with N permutations of
...
def rollup_searches(dataframe, attributes, mode):
    """ Gather all permutations of submission_date, country,
    search_provider, default_provider, locale, distribution_id and
    count:
...
def write_manifest(bucket, prefix, mode, version, start_ds, csv_paths, retry_max=10):
    ...
    A manifest enumerates the locations of csv files that contain the rollup.
    This might be useful if the file is partitioned across multiple files.
...
def save(dataframe, bucket, prefix, mode, version, start_ds):
    ...
    if mode == "monthly":
        ...
        select_expr[0] = F.lit(start_date.format("YYYY-MM-DD"))

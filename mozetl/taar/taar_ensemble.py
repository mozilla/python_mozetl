...
def verify_counts(taar_training, addons_info_df, client_samples_df):
    ...
    assert taar_training_count != client_samples_count
    # taar training should contain exactly the same number of elements in addons_info_frame
    # it should have filtered out clients that started in client_features_frame

    print("All counts verified.  taar_training_count == %d" % taar_training_count)


def extract(spark, date_from, minInstalledAddons):
    client_samples_df = get_df(spark, date_from)
    addons_info_df = get_addons_per_client(client_samples_df, minInstalledAddons)
...
def transform(spark, taar_training, regParam, elasticNetParam):
    ...
    rdd_list = [
        spark.sparkContext.union(rdd_list) for rdd_list in stacked_datasets_rdd_list
    ]
...
def load(coefs, date, prefix, bucket):
    store_json_to_s3(
        json.dumps(coefs, indent=2), "ensemble_weight_daily", date, prefix, bucket
    )
...
def main(
    ...
    """
    spark = (
        SparkSession.builder.appName("taar_ensemble").enableHiveSupport().getOrCreate()
    )

    date_from = client_sample_date_from

    taar_training = extract(spark, date_from, min_installed_addons)

...
# TEMPORARY diagnostic hook. None in normal operation; the driver sets it to a
# count_trace.Tracer. See ../count_trace.py and ../crashcorrelations_traced/README.md.
TRACE = None
...
def find_deviations(sc, reference, groups=None, signatures=None, min_support_diff=0.15, min_corr=0.03, all_addons=None, all_gfx_critical_errors=None, all_app_notes=None):
    ...
    def save_count(candidate, count, df):
        if df not in saved_counts:
            saved_counts[df] = {}
        # TEMPORARY. Every write to saved_counts goes through here, so recording the
        # previous value makes an overwrite with a smaller number visible directly.
    ...
    if signatures is not None:
        ...
        # TEMPORARY. The pivotal measurement: `results` is straight off collect(), before
        # save_results or any filtering. note_independent_counts recomputes the same numbers
        # with groupBy on dfReference, the post-augment dataframe this actually counts on.
        # If collect() is wrong but groupBy is right, the fault is in the expression above.
        if TRACE is not None:
    ...
    # was right after level 1 but wrong in the published output can be pinned to either the
    # counting or the filtering.
    if TRACE is not None:

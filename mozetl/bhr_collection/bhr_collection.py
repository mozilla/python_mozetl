...
def get_data(sc, sqlContext, config, date, end_date=None):
    ...
    try:
        result = mapped.filter(lambda p: p["application/buildId"][:8] >= date_str and p["application/buildId"][:8] <= end_date_str)
...
def filter_hang(hang):
  return hang['thread'] == 'Gecko' and len(hang['stack']) > 0 and len(hang['stack']) < 300
...
def process_hangs(ping):
    ...
    pre_result = [(
        ...
        build_date,
        platform,
    ) for h in hangs if filter_hang(h)]
...
def etl_job_incremental_write(sc, sqlContext, config=None):
    ...
    for x in range(iterations):
        ...
        profile = profile_processor.process_into_profile()
        filepath = "%s_incremental_%s" % (final_config['hang_profile_out_filename'], date_str)
